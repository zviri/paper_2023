...
def main(nodes_path: Path, edges_path: Path, output_graph: Path):
    ...
    nodes_df["label"] = nodes_df.apply(
        lambda row: "debtor" if row["person_type"] == "N" else row["category"], axis=1
    )
    ...
    logger.info("Selecting subgraph with edges in form NaturalPerson -> LegalPerson")
    nodes_data = g.nodes(data=True)
    target_edges = [
        e
        for e in g.edges
        if (
            nodes_data[e[0]]["person_type"] == "N"
            and nodes_data[e[1]]["person_type"] == "L"
        )
    ]

    retail_subgraph = g.edge_subgraph(target_edges).copy()
    assert nx.bipartite.is_bipartite(retail_subgraph)

    logger.info(f"Subgraph has {len(retail_subgraph.nodes)} nodes")
    logger.info(f"Subgraph has {len(retail_subgraph.edges)} edges")

    in_degrees_df = pd.DataFrame(
        retail_subgraph.in_degree(), columns=["id", "in_degree"]
    )
    ...
    logger.info(in_degrees_df[["id", "in_degree", "label", "name"]][:20].to_string())

    nx.write_gpickle(retail_subgraph, output_graph)
